...
func (h *PaymentHandler) Return(w http.ResponseWriter,r *http.Request){
    ...
    if h.svcCtx.Config.Payment.FrontendReturnURL!=""{u,_:=url.Parse(h.svcCtx.Config.Payment.FrontendReturnURL);q:=u.Query();q.Set("paymentNo",p.PaymentNo);q.Set("status",p.Status);u.RawQuery=q.Encode();http.Redirect(w,r,u.String(),http.StatusFound);return}
    httpx.OkJson(w,toPaymentResp(p))
}

func (h *PaymentHandler) Cancel(w http.ResponseWriter,r *http.Request){
    state:=r.URL.Query().Get("state");if state==""{httpx.Error(w,errors.New("paypal cancel state is missing"));return};_,_,paymentNo,err:=paypal.ParseState(h.paypalSecret(),state);if err!=nil{httpx.Error(w,err);return}
    if h.svcCtx.Config.Payment.FrontendReturnURL!=""{u,_:=url.Parse(h.svcCtx.Config.Payment.FrontendReturnURL);q:=u.Query();q.Set("paymentNo",paymentNo);q.Set("status","CANCELLED");u.RawQuery=q.Encode();http.Redirect(w,r,u.String(),http.StatusFound);return};httpx.OkJson(w,map[string]any{"paymentNo":paymentNo,"status":"CANCELLED"})
}
...
func (h *PaymentHandler) PayPalWebhook(w http.ResponseWriter,r *http.Request){
    ...
    var event struct{ID string `json:"id"`;EventType string `json:"event_type"`;Resource struct{CustomID string `json:"custom_id"`;Status string `json:"status"`;ID string `json:"id"`;SupplementaryData struct{RelatedIDs struct{OrderID string `json:"order_id"`} `json:"related_ids"`} `json:"supplementary_data"`} `json:"resource"`}
    if err:=json.Unmarshal(body,&event);err!=nil{httpx.Error(w,err);return}
    if event.EventType=="PAYMENT.CAPTURE.COMPLETED"&&strings.EqualFold(event.Resource.Status,"COMPLETED")&&event.Resource.CustomID!=""{
        tenantID,merchantID,paymentNo,err:=paypal.ParseState(h.paypalSecret(),event.Resource.CustomID);if err!=nil{httpx.Error(w,err);return}
        ctx:=paymentContext(r.Context(),tenantID,merchantID);if _,err:=h.svcCtx.PaymentClient.MarkPaid(ctx,&travel.MarkPaymentPaidRequest{PaymentNo:paymentNo,ProviderPaymentId:event.Resource.SupplementaryData.RelatedIDs.OrderID});err!=nil{httpx.Error(w,err);return}
    }
